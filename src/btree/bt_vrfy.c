  ...
  WT_SESSION_IMPL *session, WT_REF *ref, WT_CELL_UNPACK_ADDR *addr_unpack, WT_VSTUFF *vs)
{
    ...
    case WT_PAGE_COL_VAR:
        /*
         * VLCS trees can have WT_PAGE_COL_INT or WT_PAGE_COL_VAR pages, and gaps in the namespace
         * *are* allowed. Use the tree type to pick the check logic.
         */
        if (btree->type == BTREE_COL_VAR && ref->ref_recno < vs->records_so_far + 1)
            WT_RET_MSG(session, WT_ERROR,
...
celltype_err:
    ...
    case WT_PAGE_COL_INT:
        ...
        WT_INTL_FOREACH_BEGIN (session, page, child_ref) {
             ...
             * introduce gaps; allow a gap but not overlapping ranges.
             */
            if (btree->type == BTREE_COL_VAR && child_ref->ref_recno < vs->records_so_far + 1) {
                WT_RET_MSG(session, WT_ERROR,
