...
/*
 * Additional forward declarations for types defined with the "typedef struct" pattern
 * rather than bare "struct" (not picked up by dist/s_typedef).
 */
struct __wt_disagg_metadata;
typedef struct __wt_disagg_metadata WT_DISAGG_METADATA;
struct __wt_disagg_checkpoint_meta;
typedef struct __wt_disagg_checkpoint_meta WT_DISAGG_CHECKPOINT_META;

/*
 * Enum types used in public function signatures. Defined here so that
 * extern_noninline.h can be included without pulling in heavyweight headers.
 */
