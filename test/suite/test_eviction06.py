...
# test_eviction06.py
# Verify the per-btree dirty-index ring (WT-17236) is exercised end-to-end.
#
...
#   Consume side  cache_eviction_dirty_index_scanned -- ring slots examined
#                 cache_eviction_dirty_index_hit     -- ref pushed to LRU queue
#                 cache_eviction_dirty_index_stale   -- ref skipped (not evictable)
...
class test_eviction06(wttest.WiredTigerTestCase):
    ...
    def test_dirty_index_insert_and_drain(self):
        ...
        self.assertGreater(self.get_stat(stat.conn.cache_eviction_dirty_index_scanned), 0)
        # Every scanned slot that is non-NULL results in either a hit or a stale skip.
        self.assertGreater(
            self.get_stat(stat.conn.cache_eviction_dirty_index_hit) +
            self.get_stat(stat.conn.cache_eviction_dirty_index_stale), 0)
