...
class test_rollback_to_stable47(test_rollback_to_stable_base):
    ...
    def test_rollback_to_stable(self):
        ...
        self.prout('First checkpoint before backup')
        self.session.checkpoint()

        # Verify state before backup: 5 stable + 5 unstable records.
        self.prout('Read stable 5 keys at ts=10')
        self.check(value_a, uri, nrows_stable, 10)

        self.prout('Read unstable 5 keys at ts=30, they are present before backup-restore RTS')
        ...
        self.prout('Open from backup directory')
        ...
        self.prout('Verify stable 5 keys at ts=10')
        self.check(value_a, uri, nrows_stable, 10)
        
        self.prout('Verify unstable 5 keys at ts=30 are absent, for stable=20')
        self.session.begin_transaction("read_timestamp=" + self.timestamp_str(30))
        cursor = self.session.open_cursor(uri)
        for i in range(nrows_stable + 1, nrows + 1):
            cursor.set_key(ds.key(i))
        ...
        self.prout('First re-insert (prepared, becomes stable after stable advances)')  
        cursor = self.session.open_cursor(uri)
        self.session.begin_transaction()
        for i in range(nrows_stable + 1, nrows + 1):
        ...
        self.prout('Verify value_c is visible at ts=25')
        ...
        # Update chain: [value_d ts=35] -> [value_c ts=25 (stable)] -> [tombstone no_ts].
        self.prout('Advance oldest=30, stable=30')
        self.conn.set_timestamp(
            "oldest_timestamp=" + self.timestamp_str(30)
            + ",stable_timestamp=" + self.timestamp_str(30)
        )

        self.prout('Second checkpoint before second re-insert (REPLACED WITH sleep for DEBUG)')
        # self.session.checkpoint()  # wait for eviction server to advance oldest_id
        time.sleep(1)  # wait for eviction server to advance oldest_id


        self.prout('Second re-insert (unstable) at ts=35, and prune globally visible tombstone from backup-restore RTS')
        cursor = self.session.open_cursor(uri)
        self.session.begin_transaction()
        for i in range(nrows_stable + 1, nrows + 1):
        ...
        self.prout('Start checkpoint and evict')
        try:
            ckpt.start()
